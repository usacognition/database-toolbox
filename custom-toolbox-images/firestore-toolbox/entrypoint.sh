...
# Custom entrypoint for Firestore Google AI Toolbox
...
# Main execution
main() {
    log "Starting Firestore custom toolbox entrypoint"
    ...
    # Execute the original toolbox with Firestore prebuilt and passed arguments
    log "Executing Firestore toolbox with arguments: $*"
    exec "$ORIGINAL_ENTRYPOINT" "--prebuilt" "firestore" "$@"
}
